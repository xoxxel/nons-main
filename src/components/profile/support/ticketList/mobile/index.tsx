...
const MobileTicketList = ({ tickets }: { tickets: TicketModel[] }) => {
  return (
    <Box sx={{ width: "100%", display: { md: "none", xs: "block" } }}>
      ...
      >
        {tickets?.length > 0 ? (
          ...
          >
            {tickets?.map((ticket) => (
              <Link href={`/profile/support/${ticket?.id}`} key={ticket?.id}>
                <Box
                  sx={{
                  ...
                  >
                    <Box
                      sx={{
                        ...
                        borderRadius: "15px",
                        mb: 1.5,
                        visibility: ticket?.last_message?.is_seen ? "hidden" : "unset",
                      }}
                    ></Box>
                    <Box>
